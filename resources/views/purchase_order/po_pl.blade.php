...
@section('title', __('Purchase Order Pembelian Langsung'))
@section('custom-css')
...
@section('content')
            ...
            <div class="card">
                <div class="card-header">
                    <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#add-po"
                        onclick="addPo()"><i class="fas fa-plus"></i> Add New PO/PL</button>
    ...
    <div class="modal fade" id="detail-po">
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 id="modal-title" class="modal-title">{{ __('Detail Purchase Order Pembelian Langsung') }}</h4>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
...
@section('custom-js')
    ...
    <script>
        ...
        function lihatPo(data) {
            ...
            $.ajax({
                url: "{{ url('products/purchase_order_pl') }}",
                type: "GET",
                data: {
                    id: data.id
                },
                    ...
                    if (data?.po?.details?.length == 0) {
                        ...
                        $.each(data?.po?.details, function(index, value) {
                            ...
                            var html = '<tr>' +
                                ...
                                '</button><button title="hapus" id="delete_po_save" type="button" class="btn btn-danger btn-xs" data-id="' +
                                id +
                        ...
                        if (data?.po?.details?.length == 0 || data?.po?.details == null) {
                            ...
                            $.each(data?.po?.details, function(index,
                                ...
                                var html = '<tr>' +
                                    ...
                                    '<td>' + deskripsi + '</td>' +
                                    '<td><input type="date" value="' + newDate +
                                    '" class="form-control" id="batas' + id +
                                    '" name="batas' + id + '"></td>' +
                                    '<td>' + qty + '</td>' +
                                    '<td>' + satuan + '</td>' +
                                    '<td><input type="text" value="' + harga_per_unit +
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        $.each(data.products, function(key, value) {
                            ...
                            var checkbox
                            if (!value.id_po) { //value.id_spph penambahan optional spph untuk po/pl
